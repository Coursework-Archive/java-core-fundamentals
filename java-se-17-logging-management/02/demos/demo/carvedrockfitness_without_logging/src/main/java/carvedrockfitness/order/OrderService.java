...
public class OrderService {
    ...
    public boolean addOrder(Order order) {
        if (order.getOrderDateTime().isAfter(LocalDateTime.now())) {
            try {
                LOGGER.log(Level.WARNING, "Trying to place an order in the future, order detail: " + order);
        ...
        if (order.getProducts().size() < 1) {
            try {
                LOGGER.log(Level.WARNING, "Trying to to place an order in the future, order details: " + order);
